...
## Data specs -------------------------------------------------------------------
source ~/.bashrc
DEVN - no such file or directory
...
cd $BASE2/data-raw/fastq/samples
...
mkdir -p $BASE2/data-raw/counts
cd $BASE2/data-raw/counts
...
mkdir -p $BASE2/data-raw/aggregate
cd $BASE2/data-raw/aggregate

# run only the -all 600 sample
for I in $(ls $BASE2/data-meta/aggregate/aggregate*all.csv); do 
